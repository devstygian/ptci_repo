...
$sql = "SELECT candidateno, fullname, Course, team FROM managecandidates WHERE gender = 'Female'";
...
<form id="scoreForm" method="post" action="submitScore.php">
    <table class="table" id="candidatesTable">
        ...
        <tbody>
            <?php if (!empty($candidates)): ?>
                <?php foreach ($candidates as $candidate): ?>
                    <tr>
                        ...
                        <td>
                            <select name="scores[<?php echo $candidate['candidateno']; ?>][mastery]" class="form-control" required>
                        ...
                        <td>
                            <select name="scores[<?php echo $candidate['candidateno']; ?>][performance]" class="form-control" required>
                                <option value="" disabled selected>Select Score</option>
                                <?php for ($i = 8; $i <= 10; $i++): ?>
                                    <option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                <?php endfor; ?>
                        ...
                        <td>
                            <select name="scores[<?php echo $candidate['candidateno']; ?>][overall_impression]" class="form-control" required>
                                <option value="" disabled selected>Select Score</option>
                                <?php for ($i = 8; $i <= 10; $i++): ?>
                                    <option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                <?php endfor; ?>
                        ...
                        <td>
                            <select name="scores[<?php echo $candidate['candidateno']; ?>][audience_impact]" class="form-control" required>
                                <option value="" disabled selected>Select Score</option>
                                <?php for ($i = 8; $i <= 10; $i++): ?>
                                    <option value="<?php echo $i; ?>"><?php echo $i; ?></option>
                                <?php endfor; ?>
            ...
            <?php else: ?>
                <tr>
                    <td colspan="8" class="text-center">No female candidates found.</td>
                </tr>
            <?php endif; ?>
        </tbody>
    </table>
...
<table class="table" id="submittedScoresTable">
    ...
    <tbody>
        <?php if (!empty($submittedScores)): ?>
            <?php foreach ($submittedScores as $candidateNo => $scores): ?>
                <?php 
                ...
                <tr>
                    ...
                    <td><?php echo htmlspecialchars($scores['mastery']); ?></td>
                    <td><?php echo htmlspecialchars($scores['performance']); ?></td>
                    <td><?php echo htmlspecialchars($scores['overall_impression']); ?></td>
                    <td><?php echo htmlspecialchars($scores['audience_impact']); ?></td>
                </tr>
            <?php endforeach; ?>
        <?php else: ?>
            <tr>
                <td colspan="8" class="text-center">No scores submitted yet.</td>
            </tr>
        <?php endif; ?>
    </tbody>
</table>
